function[channel_MBS, energy_cost_for_G_O,time_cost_for_G_O] = allocate_for_G_O(channel_MBS, index_of_remote, index_of_access, p_M, channel_M, device_cpu_require, device_time, server_cpu, server_energy, W, sigma, varphi, file,K,device_per_energy,device_per_cpu)
...
%         channel_number_set_of_ue_i = [];  
        ...
        if ((channel_number_of_mbs ~= -1))
        %比较
        if ((sinr_mbs(channel_number_of_mbs) ~= -1))
