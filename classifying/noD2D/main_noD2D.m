...
for k =1 :length(K)
    for j = 1:Frame
        ...
        [channel_MBS, energy_cost_for_G_R(k),time_G_R(k)] = allocate_for_G_R(channel_MBS, index_of_remote, index_of_access, p_M, channel_M, device_cpu_require, device_time, server_cpu, server_energy, W, sigma, varphi, file,K(k));
        [channel_MBS, energy_cost_for_G_O(k),time_G_O(k)] = allocate_for_G_O(channel_MBS, index_of_remote, index_of_access, p_M, channel_M, device_cpu_require, device_time, server_cpu, server_energy, W, sigma, varphi, file,K(k),device_per_energy,device_per_cpu)
