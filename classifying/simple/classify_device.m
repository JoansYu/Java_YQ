...
for i = 1:K
    ...
    if (time(i)>device_time(i))
        index_of_remote(i) = 2;
            ...
            %计算所需的信道数
            n_mbs(i)=ceil(file(i)/((device_time(i)-device_cpu_require(i)/server_cpu(i))*W*log2(1+p_M(i)*channel_M(i)/sigma)));
            n_sbs(i)=ceil(file(i)/((device_time(i)-device_cpu_require(i)/server_cpu(i)-file(i)*varphi)*W*log2(1+p_S(i)*channel_S(i)/sigma)));
            if (energy(i)<(p_M(i)*n_mbs(i)+server_energy(i)*W/1e9)&&energy(i)<(p_S(i)*n_sbs(i)+server_energy(i)*W/1e9))
                index_of_remote(i) = 1;
            else
                index_of_remote(i) = 2; 
            end
